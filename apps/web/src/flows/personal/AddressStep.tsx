...
export function PersonalAddressStep({ onContinue, renderShell }: Props) {
  const { draft, updatePersonal } = useKyc();
  const [address, setAddress] = useState<AddressInput>(
    draft?.kind === "personal" && draft.address ? draft.address : empty,
  );
  ...
  async function onSubmit(e: FormEvent) {
    ...
    await updatePersonal({ step: "face-prime", address: parsed.data });
    onContinue();
  }
